...
import WaterAssesments from 'utils/assesments/water';
...
const AssesmentOffline = () => {
  ...
  console.log('data tfu', dataWater);
  ...
  const handleDocument = useCallback((acceptedFiles) => {
    ...
    if (!filename.includes(dataWater?.type)) {
      toast.error(`Harap masukkan file data peniliaian ${dataWater?.type}`);
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      const assesmentTemplate = WaterAssesments.find(
        (i) => i.name === dataWater?.type
      ).assesment;
  ...
  const onSubmit = async (values) => {
    ...
    await API.postWaterAssement(values, { type: dataWater?.type })
      .then((res) => {
